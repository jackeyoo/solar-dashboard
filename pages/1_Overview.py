...
if df.empty:
    ...
    def get_kpi(name):
        ...
        if row.empty:
            return 0
        val = row["value_thb"].values[0].replace(",", "").replace("%", "")
        try:
            return float(val)
        except:
            return 0
    ...
    c1.markdown(f'<div class="card"><div class="label">💰 Total Cost</div><div class="metric">฿{total_cost:,.0f}</div></div>', unsafe_allow_html=True)
    c2.markdown(f'<div class="card"><div class="label">☀️ Solar Saving</div><div class="metric">฿{solar_saving:,.0f}</div></div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="card"><div class="label">⚡ Net Cost</div><div class="metric">฿{net_cost:,.0f}</div></div>', unsafe_allow_html=True)
    c4.markdown(f'<div class="card"><div class="label">📉 Saving %</div><div class="metric">{percent:.1f}%</div></div>', unsafe_allow_html=True)
